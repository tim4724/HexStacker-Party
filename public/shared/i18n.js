...
var LOCALES = {
  en: {
    ...
    leaderboard_heading: 'Global Top 10',
    ...
    privacy_scope: 'This privacy notice applies to the HexStacker web game, to the Couch Games app (Android and iOS) through which HexStacker is launched and played, and to the HexStacker TV apps (Android TV and Apple TV) that run the game on a television.',
    ...
    privacy_data: 'data',
    privacy_data_detail: 'IP addresses are processed by the web server when you access the website. During a multiplayer session, they are also processed by our game server, which relays messages between the participating devices. When devices establish a direct connection to each other, a helper server (stun.hexstacker.com) is additionally involved to assist with that connection (no gameplay data is transmitted through it). Web server access logs are retained for a maximum of 7 days for security and operational purposes and then automatically deleted. The game server itself stores no connection data; infrastructure-level logs by our processor are retained solely for security and operational purposes and no longer than necessary for those purposes. The helper server retains no data. When you join via the Couch Games app, the scanned or entered room code is additionally sent to our game server to look up the corresponding game room; only the room code and — as technically required — your IP address are transmitted, not your player name. The HexStacker TV apps connect to the game server as the display that opens a game room and, where controllers connect directly, to the helper server; as with any participating device, their IP address is processed for these connections. They do not access the website.',
    ...
    privacy_tracking_detail: 'No cookies, no analytics or advertising services, and no third-party trackers are used. Settings strictly necessary for service operation within the meaning of Section 25(2) No. 2 TDDDG (German Telecommunications-Digital-Services Data Protection Act) are stored locally in the browser (e.g. the last-entered player name, chosen color, mute state, further display preferences, and a reconnection ID scoped to the current game room). Local storage is not a tracking mechanism; it merely retains these settings for the next session. The Couch Games app likewise stores a few items solely on your device for convenience: the last-used player name and the most recently joined game room (room code and its link), which is automatically deleted after at most 12 hours. This storage is strictly necessary for operation (Section 25(2) No. 2 TDDDG) and is not used for tracking. The HexStacker TV apps store no data on the device at all: settings such as the mute state exist only for the current session and are not persisted.',
    privacy_camera: 'camera and QR scan (app)',
    privacy_camera_detail: 'The Couch Games app may use the camera of your device to scan the game-room QR code shown on your TV or screen. The camera image is processed solely on the device; no images are stored or transmitted. Only the room link contained in the QR code is read. On both iOS and Android you are asked for camera permission before the first scan (legal basis: consent, Art. 6(1)(a) GDPR). On iOS scanning uses the built-in Apple camera functionality; on Android it happens directly in the app using an embedded scanning library (Google ML Kit) that recognizes the QR code entirely on the device. Camera images are never transmitted to Google or any other third party.',
    ...
    privacy_appstores: 'app stores',
    privacy_appstores_detail: 'The Couch Games app and the HexStacker TV apps are distributed via the Apple App Store and Google Play. When you download and use them, these platforms process their own data (e.g. download and, where applicable, diagnostic data) under their own responsibility and privacy policies. The apps themselves contain no analytics, advertising, or tracking services, no third-party crash reporting, and no device or advertising identifiers.',
    privacy_hosting: 'hosting',
    privacy_hosting_detail: 'The website and the helper server (stun.hexstacker.com) are hosted on servers in Germany operated by the controller. The game servers used for multiplayer and room lookup (ws.hexstacker.com and ws.couch-games.com) are operated for us by Fly.io, Inc. (Chicago, USA) as a processor under Art. 28 GDPR. Because the game servers run in multiple regions, traffic may pass through locations outside the EEA; such transfers are safeguarded by the EU Standard Contractual Clauses (Decision (EU) 2021/914, Module 2 pursuant to Art. 46(2)(c) GDPR) and supplementary technical measures (TLS encryption in transit). Fly.io, Inc. is also certified under the EU-US Data Privacy Framework (adequacy decision (EU) 2023/1795). A copy of the Standard Contractual Clauses is available on request.',
  ...
  de: {
    ...
    leaderboard_heading: 'Globale Top 10',
    leaderboard_alltime: 'Allzeit',
    ...
    privacy_scope: 'Diese Datenschutzerklärung gilt für das Web-Spiel HexStacker, für die Couch-Games-App (Android und iOS), über die HexStacker gestartet und gespielt wird, sowie für die HexStacker-TV-Apps (Android TV und Apple TV), die das Spiel auf einem Fernseher darstellen.',
    ...
    privacy_data: 'Daten',
    privacy_data_detail: 'IP-Adressen werden vom Webserver beim Aufruf der Website verarbeitet. Während einer Multiplayer-Sitzung werden sie zudem von unserem Spielserver verarbeitet, der Nachrichten zwischen den Geräten vermittelt. Bei direkten Verbindungen zwischen Spielgeräten kommt ein Hilfsserver (stun.hexstacker.com) hinzu, der den Geräten beim Verbindungsaufbau hilft (es werden keine Spieldaten darüber übertragen). Zugriffslogs des Webservers werden zu Sicherheits- und Betriebszwecken für maximal 7 Tage gespeichert und anschließend automatisch gelöscht. Der Spielserver speichert selbst keine Verbindungsdaten; infrastrukturseitige Logs unseres Auftragsverarbeiters werden ausschließlich zu Sicherheits- und Betriebszwecken aufbewahrt und nicht länger, als für diese Zwecke erforderlich. Der Hilfsserver speichert keine Daten. Beim Beitritt über die Couch-Games-App wird zusätzlich der gescannte oder eingegebene Raumcode an unseren Spielserver übermittelt, um den zugehörigen Spielraum zu ermitteln; dabei werden nur der Raumcode und – technisch bedingt – Ihre IP-Adresse übertragen, nicht Ihr Spielername. Die HexStacker-TV-Apps verbinden sich als Anzeige, die einen Spielraum eröffnet, mit dem Spielserver und – bei direkten Controller-Verbindungen – mit dem Hilfsserver; wie bei jedem beteiligten Gerät wird ihre IP-Adresse für diese Verbindungen verarbeitet. Sie rufen keine Website auf.',
    ...
    privacy_tracking_detail: 'Es werden keine Cookies, keine Analyse- oder Werbedienste und keine Drittanbieter-Tracker eingesetzt. Für den Spielbetrieb unbedingt erforderlich i.S.v. § 25 Abs. 2 Nr. 2 TDDDG ist die lokale Speicherung weniger Einstellungen im Browser des Endgeräts (z. B. zuletzt eingegebener Spielername, gewählte Spielfarbe, Stummschaltung, weitere Anzeigeeinstellungen und eine Wiederverbindungs-ID, die nur für den aktuellen Spielraum gilt). Die lokale Speicherung ist keine Tracking-Maßnahme; sie dient lediglich der Vorhaltung dieser Einstellungen für die nächste Sitzung. Auch die Couch-Games-App speichert einige Angaben ausschließlich lokal auf dem Gerät, um den Bedienkomfort zu wahren: den zuletzt verwendeten Spielernamen sowie den zuletzt beigetretenen Spielraum (Raumcode und zugehöriger Link), der automatisch spätestens nach 12 Stunden gelöscht wird. Diese Speicherung ist für den Betrieb unbedingt erforderlich (§ 25 Abs. 2 Nr. 2 TDDDG) und dient nicht dem Tracking. Die HexStacker-TV-Apps speichern überhaupt keine Daten auf dem Gerät: Einstellungen wie die Stummschaltung bestehen nur für die aktuelle Sitzung und werden nicht dauerhaft gespeichert.',
    privacy_camera: 'Kamera und QR-Scan (App)',
    privacy_camera_detail: 'Die Couch-Games-App kann die Kamera Ihres Geräts verwenden, um den auf dem Fernseher oder Bildschirm angezeigten QR-Code eines Spielraums zu scannen. Das Kamerabild wird ausschließlich lokal auf dem Gerät verarbeitet; es werden keine Bilder gespeichert oder übertragen. Ausgewertet wird nur der im QR-Code enthaltene Raum-Link. Sowohl auf iOS als auch auf Android werden Sie vor dem ersten Scan um Erlaubnis für den Kamerazugriff gebeten (Rechtsgrundlage: Einwilligung, Art. 6 Abs. 1 lit. a DSGVO). Auf iOS erfolgt der Scan über die systemeigene Kamerafunktion von Apple; auf Android erfolgt er direkt in der App über eine eingebundene Scan-Bibliothek (Google ML Kit), die den QR-Code vollständig lokal auf dem Gerät erkennt. Kamerabilder werden dabei nicht an Google oder sonstige Dritte übertragen.',
    ...
    privacy_appstores: 'App-Stores',
    privacy_appstores_detail: 'Die Couch-Games-App und die HexStacker-TV-Apps werden über den Apple App Store und Google Play bereitgestellt. Beim Herunterladen und bei der Nutzung verarbeiten diese Plattformen in eigener Verantwortung eigene Daten (z. B. Download- und ggf. Diagnosedaten) nach ihren jeweiligen Datenschutzbestimmungen. Die Apps selbst enthalten keine Analyse-, Werbe- oder Tracking-Dienste, keine Absturzberichterstattung an Dritte und keine Geräte- oder Werbekennungen.',
    privacy_hosting: 'Hosting',
    privacy_hosting_detail: 'Die Website und der Hilfsserver (stun.hexstacker.com) werden auf Servern des Verantwortlichen in Deutschland gehostet. Die für Multiplayer und Raumsuche genutzten Spielserver (ws.hexstacker.com und ws.couch-games.com) werden für uns von Fly.io, Inc. (Chicago, USA) als Auftragsverarbeiter nach Art. 28 DSGVO betrieben. Da die Spielserver mehrregional laufen, kann Datenverkehr auch über Standorte außerhalb des EWR geroutet werden; solche Übermittlungen sind durch die EU-Standardvertragsklauseln (Beschluss (EU) 2021/914, Modul 2 nach Art. 46 Abs. 2 lit. c DSGVO) sowie ergänzende technische Maßnahmen (TLS-Verschlüsselung in Transit) abgesichert. Fly.io, Inc. ist zudem nach dem EU-US Data Privacy Framework (Angemessenheitsbeschluss (EU) 2023/1795) zertifiziert. Eine Kopie der Standardvertragsklauseln stellen wir auf Anfrage bereit.',
    ...
    privacy_updated: 'Zuletzt aktualisiert Juli 2026',
  ...
  fr: {
    ...
    leaderboard_heading: 'Top 10 mondial',
    leaderboard_alltime: 'Général',
    leaderboard_monthly: 'Mensuel',
  ...
  pt: {
    ...
    leaderboard_heading: 'Top 10 global',
    leaderboard_alltime: 'Geral',
    leaderboard_monthly: 'Mensal',
  ...
  zh: {
    ...
    new_player: '新玩家',
    leaderboard_heading: '全球前 10 名',
    leaderboard_alltime: '总榜',
    leaderboard_monthly: '月榜',
    color_choose: '选择颜色 {n}',
  ...
  ja: {
    ...
    new_player: '新しいプレイヤー',
    leaderboard_heading: '世界トップ10',
    leaderboard_alltime: '総合',
    leaderboard_monthly: '月間',
  ...
  ko: {
    ...
    new_player: '새 플레이어',
    leaderboard_heading: '글로벌 톱 10',
    leaderboard_alltime: '전체',
    leaderboard_monthly: '월간',
    color_choose: '색 {n} 선택',
  ...
  ru: {
    ...
    leaderboard_heading: 'Топ-10 мира',
    leaderboard_alltime: 'За всё время',
    leaderboard_monthly: 'За месяц',
  ...
  it: {
    ...
    leaderboard_heading: 'Top 10 mondiale',
    leaderboard_alltime: 'Generale',
  ...
  tr: {
    ...
    leaderboard_heading: 'Dünya İlk 10',
