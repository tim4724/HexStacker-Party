...
var LOCALES = {
  en: {
    ...
    privacy_data: 'data',
    privacy_data_detail: 'IP addresses are processed by the web server and the WebSocket relay to serve the site and enable multiplayer communication. Web server access logs are retained for a maximum of 7 days for security purposes (abuse detection and operational troubleshooting) and then automatically deleted; relay logs are governed by our processor’s retention policy. Logs are not shared with third parties. Processing is based on legitimate interest in operating the service (Art. 6(1)(f) GDPR).',
    ...
    privacy_hosting_detail: 'The website is hosted on servers in Germany operated by the controller. The WebSocket relay used for multiplayer (ws.hexstacker.com) is operated for us by Fly.io, Inc. (Chicago, USA) as a processor under Art. 28 GDPR. Because the relay runs in multiple regions, traffic may pass through locations outside the EEA; such transfers are safeguarded by the EU Standard Contractual Clauses (Art. 46(2)(c) GDPR). A copy of the clauses is available on request.',
  ...
  de: {
    ...
    privacy_data: 'Daten',
    privacy_data_detail: 'IP-Adressen werden vom Webserver und dem WebSocket-Relay zur Bereitstellung der Seite und zur Ermöglichung der Multiplayer-Kommunikation verarbeitet. Zugriffslogs des Webservers werden zu Sicherheitszwecken (Missbrauchserkennung und Betriebsanalyse) für maximal 7 Tage gespeichert und anschließend automatisch gelöscht; Logs des Relays unterliegen der Aufbewahrungspraxis unseres Auftragsverarbeiters. Die Logs werden nicht an Dritte weitergegeben. Die Verarbeitung erfolgt auf Grundlage des berechtigten Interesses am Betrieb des Dienstes (Art. 6 Abs. 1 lit. f DSGVO).',
    privacy_tracking: 'Tracking',
