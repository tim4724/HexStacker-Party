...
var LOCALES = {
  en: {
    ...
    privacy_data: 'data',
    privacy_data_detail: 'IP addresses are processed by the web server, the WebSocket relay, and — when direct peer-to-peer connections are established between game devices — by the STUN server stun.hexstacker.com (operated by the controller; see Hosting). Direct peer-to-peer connections expose participating devices’ IP addresses to each other; modern browsers typically anonymize local addresses as .local mDNS names. Web server access logs are retained for a maximum of 7 days for security purposes (abuse detection and operational troubleshooting) and then automatically deleted; relay logs are governed by our processor’s retention policy. The STUN server stores no data persistently. Logs are not shared with third parties. Processing is based on legitimate interest in operating the service (Art. 6(1)(f) GDPR).',
    privacy_tracking: 'tracking',
    privacy_tracking_detail: 'No cookies, no analytics, no tracking-related third-party services, and no tracking-related local storage are used.',
    privacy_gameplay: 'gameplay',
    privacy_gameplay_detail: 'Player names and inputs are transmitted between players during a session and exist only in memory. Nothing is stored. For lower input latency, the game may optionally establish a direct peer-to-peer connection (WebRTC DataChannel) between a controller device and the display device; when this connection is open, touch inputs travel directly between the two devices without passing through the relay. If the direct connection cannot be established, traffic continues to flow via the relay. Processing is based on legitimate interest in operating the service (Art. 6(1)(f) GDPR).',
    privacy_hosting: 'hosting',
    privacy_hosting_detail: 'The website and the STUN server (stun.hexstacker.com) are hosted on servers in Germany operated by the controller. The WebSocket relay used for multiplayer (ws.hexstacker.com) is operated for us by Fly.io, Inc. (Chicago, USA) as a processor under Art. 28 GDPR. Because the relay runs in multiple regions, traffic may pass through locations outside the EEA; such transfers are safeguarded by the EU Standard Contractual Clauses (Art. 46(2)(c) GDPR). A copy of the clauses is available on request.',
    privacy_rights: 'your rights',
    privacy_rights_detail: 'You have the right to access (Art. 15 GDPR), rectification (Art. 16), erasure (Art. 17), and restriction of processing (Art. 18). To exercise these rights, contact the controller at the e-mail address above. Because the service has no user accounts and stores no gameplay data, most requests will simply confirm that we hold nothing about you. Your separate right to object under Art. 21 GDPR is set out below.',
  ...
  de: {
    ...
    privacy_data: 'Daten',
    privacy_data_detail: 'IP-Adressen werden vom Webserver, dem WebSocket-Relay und — beim Aufbau direkter Peer-to-Peer-Verbindungen zwischen Spielgeräten — vom STUN-Server stun.hexstacker.com (wird vom Verantwortlichen selbst betrieben; siehe Hosting) verarbeitet. Direkte Peer-to-Peer-Verbindungen geben den beteiligten Geräten gegenseitig ihre IP-Adressen bekannt; moderne Browser anonymisieren lokale Adressen in der Regel als .local-mDNS-Namen. Zugriffslogs des Webservers werden zu Sicherheitszwecken (Missbrauchserkennung und Betriebsanalyse) für maximal 7 Tage gespeichert und anschließend automatisch gelöscht; Logs des Relays unterliegen der Aufbewahrungspraxis unseres Auftragsverarbeiters. Der STUN-Server speichert keine Daten persistent. Die Logs werden nicht an Dritte weitergegeben. Die Verarbeitung erfolgt auf Grundlage des berechtigten Interesses am Betrieb des Dienstes (Art. 6 Abs. 1 lit. f DSGVO).',
    privacy_tracking: 'Tracking',
    privacy_tracking_detail: 'Es werden keine Cookies, keine Analysedienste, keine tracking-bezogenen Drittanbieterdienste und kein Tracking-bezogener Local Storage eingesetzt.',
    privacy_gameplay: 'Spielbetrieb',
    privacy_gameplay_detail: 'Spielernamen und Eingaben werden während einer Sitzung zwischen den Spielern übertragen und existieren ausschließlich im Arbeitsspeicher. Es wird nichts gespeichert. Für niedrigere Eingabelatenz kann das Spiel optional eine direkte Peer-to-Peer-Verbindung (WebRTC DataChannel) zwischen Steuergerät und Anzeigegerät aufbauen; ist diese Verbindung offen, werden Touch-Eingaben direkt zwischen den Geräten übertragen, ohne das Relay zu passieren. Gelingt der direkte Verbindungsaufbau nicht, läuft der Datenverkehr weiterhin über das Relay. Die Verarbeitung erfolgt auf Grundlage des berechtigten Interesses am Betrieb des Dienstes (Art. 6 Abs. 1 lit. f DSGVO).',
    privacy_hosting: 'Hosting',
    privacy_hosting_detail: 'Die Website und der STUN-Server (stun.hexstacker.com) werden auf Servern des Verantwortlichen in Deutschland gehostet. Das für Multiplayer genutzte WebSocket-Relay (ws.hexstacker.com) wird für uns von Fly.io, Inc. (Chicago, USA) als Auftragsverarbeiter nach Art. 28 DSGVO betrieben. Da das Relay mehrregional läuft, kann Datenverkehr auch über Standorte außerhalb des EWR geroutet werden; solche Übermittlungen sind durch die EU-Standardvertragsklauseln (Art. 46 Abs. 2 lit. c DSGVO) abgesichert. Eine Kopie der Klauseln stellen wir auf Anfrage bereit.',
    privacy_rights: 'Rechte der Betroffenen',
    privacy_rights_detail: 'Sie haben das Recht auf Auskunft (Art. 15 DSGVO), Berichtigung (Art. 16), Löschung (Art. 17) sowie Einschränkung der Verarbeitung (Art. 18). Zur Ausübung dieser Rechte wenden Sie sich bitte an den Verantwortlichen unter der oben angegebenen E-Mail-Adresse. Da der Dienst ohne Nutzerkonten betrieben wird und keine Spieldaten speichert, werden die meisten Anfragen lediglich bestätigen, dass wir nichts über Sie gespeichert haben. Ihr gesondertes Widerspruchsrecht nach Art. 21 DSGVO finden Sie weiter unten.',
