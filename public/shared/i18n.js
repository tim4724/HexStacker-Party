...
var LOCALES = {
  en: {
    ...
    privacy_camera_detail: 'The Couch Games app may use the camera of your device to scan the game-room QR code shown on your TV or screen. The camera image is processed solely on the device; no images are stored or transmitted. Only the room link contained in the QR code is read. On iOS you are asked for camera permission before the first scan (legal basis: consent, Art. 6(1)(a) GDPR); scanning uses the built-in Apple camera functionality. On Android the scan is performed by the Google Play Services code scanner, which is provided by Google, runs locally on the device, and requires no separate camera permission from the app.',
  ...
  de: {
    ...
    privacy_camera_detail: 'Die Couch-Games-App kann die Kamera Ihres Geräts verwenden, um den auf dem Fernseher oder Bildschirm angezeigten QR-Code eines Spielraums zu scannen. Das Kamerabild wird ausschließlich lokal auf dem Gerät verarbeitet; es werden keine Bilder gespeichert oder übertragen. Ausgewertet wird nur der im QR-Code enthaltene Raum-Link. Auf iOS werden Sie vor dem ersten Scan um Erlaubnis für den Kamerazugriff gebeten (Rechtsgrundlage: Einwilligung, Art. 6 Abs. 1 lit. a DSGVO); der Scan erfolgt über die systemeigene Kamerafunktion von Apple. Auf Android wird der Scan vom Google-Play-Services-Codescanner ausgeführt, der von Google bereitgestellt wird, lokal auf dem Gerät arbeitet und keine gesonderte Kameraberechtigung der App erfordert.',
