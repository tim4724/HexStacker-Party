...
- Portable native core: `server/PartyCore.js` + the engine + `partyplug/RoomFlow.js` build to `dist/partycore.js` (iife `HexCore`) for tvOS/Android TV. Keep it pure (no DOM/timers/clock/IO) — gated by `tests/portable-purity.test.js` (static) and `tests/core-bundle-runtime.test.js` (runtime, bare VM)
